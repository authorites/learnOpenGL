...
int main() {
	...
	*/
	...
	// construct shader
	Shader ourShader("res/shader/3.3.shader.vert", "res/shader/3.3.shader.frag");
	...
	*/
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// pass them to the shaders (3different ways)
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glUniformMatrix4fv(viewLoc, 1, GL_FALSE, &view[0][0]);
		// note: currently we set the projection matrix each frame, but since the projection natrix rarely changes it's often best practice to set it outside the main loop only once.
